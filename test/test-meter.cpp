...
TEST_CASE("Test configuration", "[meter][parameters][setChannel]") {
    ...
    SECTION ("Per channel data"){
        ...
        loudness::Meter<loudness::Mode::EBU_M | loudness::Mode::TruePeak> meter(loudness::NumChannels(num_channels), min_samplerate);
...
TEST_CASE("Resetting the meter", "[meter][reset]"){
    ...
    constexpr int channels = 2;
    constexpr long num_samples = 3*samplerate;
    loudness::Meter<loudness::Mode::TruePeak | loudness::Mode::EBU_I | loudness::Mode::EBU_S | loudness::Mode::EBU_LRA> meter{loudness::NumChannels(channels), loudness::Samplerate(samplerate)};
...
TEMPLATE_LIST_TEST_CASE("At sample rate >= 192000 true peak == sample peak", "[meter][true-peak][sample-peak]", DataTypes){
...
TEMPLATE_LIST_TEST_CASE("Test all input data configurations", "[meter][input]", DataTypes){
    ...
    SECTION("Loudness measurments"){
        ...
        SECTION("Test channel based"){
            const auto channel_audio = sineWaveChannels<TestType>(1000.0, samplerate.get(), num_samples, channels.get(), target);
    ...
    SECTION ("Peak measurements"){
        ...
        SECTION ("Test interleaved"){
            ...
            const auto sine_wave = sineWaveTP<TestType>(samplerate.get()/4.0, samplerate.get(), channels.get(), 45.0, 0.5);
        ...
        SECTION ("Test deinterleaved"){
            loudness::Meter<loudness::Mode::TruePeak> meter{channels, samplerate};
            const auto interleaved = sineWaveTP<TestType>(samplerate.get()/4.0, samplerate.get(), channels.get(), 45.0, 0.5);
...
TEMPLATE_TEST_CASE_SIG("Test multi-global-loudness", "[integrated][multi]",
                        ...
                        (float, loudness::Mode::EBU_I),
                        (double, loudness::Mode::EBU_I),
                        (int16_t, loudness::Mode::EBU_I),
                        (int32_t, loudness::Mode::EBU_I),
                        (float, loudness::Mode::EBU_I | loudness::Mode::Histogram),
